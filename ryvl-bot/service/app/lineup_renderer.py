...
def _draw_pitch(draw: ImageDraw.ImageDraw, pitch_color: str) -> None:
    ...
    stripe_h = int((HEIGHT - (HEADER_HEIGHT + 50)) / 11)
...
def _draw_header(
    ...
    title_font = _fit_font(draw, title, title_max, start=38, minimum=20)
...
def _draw_player(draw: ImageDraw.ImageDraw, x: int, y: int, position_label: str, player_name: str, index: int, primary: str, secondary: str) -> None:
    ...
    font = _fit_font(draw, display_name, 150, start=22, minimum=12)
...
def render_lineup_png(
    *,
    ...
    del width, height, show_slot_tags
    ...
    for index, position in enumerate(layout.positions):
        ...
        _draw_player(draw, x, y + 108, position.label, display, index, primary, secondary)
