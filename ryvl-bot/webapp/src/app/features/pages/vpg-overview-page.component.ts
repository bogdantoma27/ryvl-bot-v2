    ...
    <section class="collection-page page-viewport space-y-6">
      ...
      <div class="wizard-panel">
        ...
        <div class="manage-form-grid">
          ...
          <label class="field"><span>Season</span><select [(ngModel)]="selectedSeason" [disabled]="previewLoading()"><option [ngValue]="null">Latest</option>@for (season of seasons(); track season) {<option [ngValue]="season">{{ season }}</option>}</select></label>
        ...
        <div class="channel-row">
          ...
          <div class="action-row" style="margin-top: 0;">
            <button class="secondary-action" type="button" (click)="preview()" [disabled]="previewLoading() || !selectedSlug">{{ previewLoading() ? 'Rendering...' : 'Preview' }}</button>
...
export class VpgOverviewPageComponent implements OnInit {
  ...
  selectLeague(slug: string): void {
    ...
    if (!slug) return;
    this.loading.set(true);
    this.api.listVpgSeasons(slug).then(items => { this.seasons.set(items); this.selectedSeason = items.at(-1) ?? null; }).catch(() => this.snackbar.error('Unable to load VPG seasons.')).finally(() => this.loading.set(false));
  }
  ...
  preview(): void {
    const season = this.selectedSeason ?? this.seasons().at(-1);
    if (!this.selectedSlug || season === undefined || this.previewLoading()) return;
    this.previewLoading.set(true);
    this.previewError.set('');
    this.api.previewVpg({ league_slug: this.selectedSlug, content_type: this.mode(), season }).then(blob => {
  ...
  postNow(): void {
    const season = this.selectedSeason ?? this.seasons().at(-1);
    if (!this.selectedSlug || season === undefined || !this.postChannelId || this.posting()) return;
    this.posting.set(true);
    this.api.postVpg({ league_slug: this.selectedSlug, content_type: this.mode(), season, channel_id: this.postChannelId }).then(result => this.snackbar.success(`Posted to channel ${result.channel_id}.`)).catch(() => this.snackbar.error('Unable to post the VPG image.')).finally(() => this.posting.set(false));
  }
