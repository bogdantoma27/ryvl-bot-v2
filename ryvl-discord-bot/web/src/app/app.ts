...
@Component({
  ...
  template: `
    @if (isAuthChecking()) {
      ...
      <!-- Unauthenticated State: Centered Login Page for Protected Routes -->
      ...
      <!-- Public Layout for Club Tracker -->
      <div class="min-h-screen bg-[#1a1a2e] text-[#dcddde] flex flex-col">
        <!-- Top Navbar -->
        <header class="bg-[#16213e] border-b border-slate-800 px-6 py-3.5 flex items-center justify-between sticky top-0 z-40 backdrop-blur-md">
          <div class="flex items-center gap-3">
            <div class="w-9 h-9 rounded-xl bg-[#5865F2] flex items-center justify-center shadow-md shadow-indigo-500/20">
              <svg class="w-5 h-5 text-white" viewBox="0 0 24 24" fill="currentColor">
                <path d="M20.317 4.37a19.791 19.791 0 0 0-4.885-1.515.074.074 0 0 0-.079.037c-.21.375-.444.864-.608 1.25a18.27 18.27 0 0 0-5.487 0 12.64 12.64 0 0 0-.617-1.25.077.077 0 0 0-.079-.037A19.736 19.736 0 0 0 3.677 4.37a.07.07 0 0 0-.032.027C.533 9.046-.32 13.58.099 18.057a.082.082 0 0 0 .031.057 19.9 19.9 0 0 0 5.993 3.03.078.078 0 0 0 .084-.028c.462-.63.874-1.295 1.226-1.994.021-.041.001-.09-.041-.106a13.107 13.107 0 0 1-1.872-.892.077.077 0 0 1-.008-.128 10.2 10.2 0 0 0 .372-.292.074.074 0 0 1 .077-.01c3.929 1.793 8.18 1.793 12.061 0a.074.074 0 0 1 .078.01c.12.098.246.198.373.292a.077.077 0 0 1-.006.127 12.299 12.299 0 0 1-1.873.893.077.077 0 0 0-.041.107c.36.698.772 1.362 1.225 1.993a.076.076 0 0 0 .084.028 19.839 19.839 0 0 0 6.002-3.03.077.077 0 0 0 .032-.054c.5-5.177-.838-9.674-3.549-13.66a.061.061 0 0 0-.031-.028z" />
              </svg>
            </div>
            <div>
              <span class="text-sm font-black text-white tracking-tight">RYVL ESPORTS</span>
              <span class="text-[10px] text-emerald-400 font-semibold ml-2 uppercase tracking-wider">
                {{ currentPath().startsWith('/transfers') ? 'VPG Superliga Transfers' : 'Pro Clubs Match Tracker' }}
              </span>
            </div>
          </div>

          <div class="flex items-center gap-3">
            <nav class="hidden sm:flex items-center gap-1 bg-[#11192e] p-1 rounded-xl border border-slate-800">
              <a
                routerLink="/club"
                routerLinkActive="bg-[#00d26a] text-black font-bold shadow-sm"
                class="px-3 py-1 rounded-lg text-xs font-semibold text-slate-300 hover:text-white transition"
              >
                Pro Clubs Tracker
              </a>
              <a
                routerLink="/transfers"
                routerLinkActive="bg-[#1f8b4c] text-white font-bold shadow-sm"
                class="px-3 py-1 rounded-lg text-xs font-semibold text-slate-300 hover:text-white transition"
              >
                VPG Transfers
              </a>
            ...
            <button
              type="button"
              (click)="loginWithDiscord()"
              class="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-xl bg-[#5865F2] hover:bg-[#4752C4] text-white text-xs font-semibold shadow-md shadow-indigo-500/20 transition cursor-pointer"
            >
...
export class App implements OnInit {
  ...
  readonly isPublicRoute = computed(
    () => this.currentPath().startsWith('/club') || this.currentPath().startsWith('/transfers'),
  );
