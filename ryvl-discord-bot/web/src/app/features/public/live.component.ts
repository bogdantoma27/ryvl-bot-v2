...
@Component({
  ...
  imports: [MatchesPanelComponent, PublicPageHeaderComponent],
  template: `
    <div class="public-page space-y-8">
      <app-public-page-header heading="Match Center" eyebrow="VPG Superliga România" description="Today’s fixtures and confirmed results.">
        <button header-actions type="button" class="public-button" (click)="refresh()" [disabled]="isLoading()">{{ isLoading() ? 'Refreshing…' : 'Refresh' }}</button>
